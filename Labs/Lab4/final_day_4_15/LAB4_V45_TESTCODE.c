...
void LCD_Print(void) {
	...
	//	pause();
		...
		if(Counts==1){	//only call the battery voltage once every second
			...
			battery =((1000*battery)/256)*15;	//Reads once it has stabilized do calc
			keypad = read_AD_input(5);	//Allow it stabilize. using this variable as just a throw away
			Counts=0;
		}//end if counts
		lcd_print("\rVoltage:%d", 10*battery);	//prints battery voltage to nearest volt
	}//end if new print
}//end LCD print
...
{					// Desired gain is chosen by incrementing it by pressing button 8 and decrementing it by pressing button 2
	...
	while (keypad != 35) 
	{
		...
		lcd_print("\rGain is: %d", steer_gain); // print gain
		keypad = read_keypad();
		pause();
		if (keypad == 50) 
		{
