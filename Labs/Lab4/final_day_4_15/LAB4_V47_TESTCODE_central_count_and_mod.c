...
//************TIMER VARIBLES*******************
volatile unsigned char hr_count=0;
...
signed int error1;
//signed int error2;//error values of servo
...
void start_run(void){	//function that does the inital thngs like get heading, gain, set values to neutral
	desired_heading=900;//desired_heading = choose_heading();                 
	ratio=.417; //choose_gain();
	pause();
	choose_speed();
...
//-----------------------------------------------------------------------------------
void Heading(void) {
	if (count_flag==1){ //new_heading){ //20 ms passed
		count_flag=0;
		heading = ReadCompass();
		//new_heading = 0;
	}//end if new heding
...
void Ranger(void)  {
	if (count_flag==2){//new_range){ //80ms passed
		count_flag=0;
		printf("\r\n new range");
		//new_range = 0;
		range=ReadRanger();
...
//-------------------------------------------------------------------------------------
void LCD_Print(void) {
	if (count_flag==4){//new_print){ // Call display function every 400 ms 
		count_flag=0;
		//new_print =0;
		lcd_clear();
	//	pause();
		...
		if(count_flag==3){//Counts==1){	//only call the battery voltage once every second
			count_flag==0;
...
void Steering_Servo() {	//function that steers using compass
	...
	if (abs(error1)<=abs(error1+3600))  {
		...
		STEER_PW = ratio*(error1+3600) + PW_CENTER;
...
void PCA_ISR(void) __interrupt 9  {   
	if (CF)  {         
		...
		hr_count++;  
		//n_count++;
		if(hr_count%2==0){//40ms for compass
			count_flag=1;
		}
		if(hr_count%4==0){//80ms for ranger
			count_flag==2;
		}
		if(hr_count%20==0){//400ms for the lcd
			count_flag=4;
		}
		if(hr_count%50==0){//1 second for battery
			count_flag=3;
			if(hr_count==100){
				hr_count=0;
			}
