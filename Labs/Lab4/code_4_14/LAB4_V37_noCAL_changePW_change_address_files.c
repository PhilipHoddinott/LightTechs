...
void main(void) {         
	...
	while (1) { 
		...
		while(!SS){	//while the slideswitch is on
			...
			printf("\n\r Range:%d  Compass:%d ", range, heading);	//print these on the secure crt for data aquisition
		}//end slide switch on
...
void start_run(void){	//function that does the inital thngs like get heading, gain, set values to neutral
	...
	MOTOR_PW_AND_STEER_PW=PW_CENTER;			//start off with the wheels straight
...
void slide_switch_off(void){//function that does stuff when the slide swithc is off
	...
	PCA0CP2 = 0xFFFF - MOTOR_PW_AND_STEER_PW;
	MOTOR_PW_AND_STEER_PW=PW_CENTER;			//run switch is off, might as well turn straight now too.
	PCA0CPL0 = 0xFFFF - MOTOR_PW_AND_STEER_PW;
	PCA0CPH0 = (0xFFFF - MOTOR_PW_AND_STEER_PW) >> 8;   
	lcd_clear();
...
void Steering_Servo() {	//function that steers using compass
	...
	if(range>45){	//if obstacle in range but not dangerous yet
		...
		if (abs(error1)<=abs(error2))  {
			MOTOR_PW_AND_STEER_PW = ratio*error1 + PW_CENTER;	//use the smaller error to calcualte the servo setting
		} //end if use error1
		else  {
			MOTOR_PW_AND_STEER_PW = ratio*error2 + PW_CENTER;
		}//end else if use error2
		if (MOTOR_PW_AND_STEER_PW < PW_MIN)  {	//if steering is set to below the minimum, set servo to minum postion
			MOTOR_PW_AND_STEER_PW = PW_MIN;
		}//end if pW < min
		if (MOTOR_PW_AND_STEER_PW > PW_MAX)  { //if steering is set to above the maximum, set servo to max postion
			MOTOR_PW_AND_STEER_PW = PW_MAX;
		}//end if PW>max

	}//end if range = far
	if(range<=45){	//if an obstacle is getting near 
		MOTOR_PW_AND_STEER_PW=PW_MAX;	//full right swerve to avoid it
	}//end if range = close
	
	PCA0CPL0 = 0xFFFF - MOTOR_PW_AND_STEER_PW;
	PCA0CPH0 = (0xFFFF - MOTOR_PW_AND_STEER_PW) >> 8;	//set servo values
}//end steering servo 
