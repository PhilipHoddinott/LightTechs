/*
is it set to east then run south
what is the range that things change at
	remove from steer function
	sould we have the debounce in there

need to ensure that it will never go backwards
...
void LCD_Print(void) {
	if (new_print){ // Call display function every 400 ms 
		...
		if(Counts==1){	//only call the battery voltage once every second
			...
			keypad = read_AD_input(6);	//Allow it stabilize. using this variable as just a throw away
			Counts=0;
		}//end if counts
