...
void main(void) {         
	...
	while (1) { 
		...
		while(!SS){	//while the slideswitch is on
			...
			printf("\n\r Range:%d  Compass:%d  PWM:%d, PW:%d ad %d", range, heading, MOTOR_PW,PW, read_AD_input(4));	//print these on the secure crt for data aquisition
		}//end slide switch on
...
void Steering_Servo() {	//function that steers using compass
	...
	if(range>45){	//if obstacle in range but not dangerous yet
	    ...
	    //ratio=.471;
	    /***********************************************
	    I MOVED THIS TO THE TOP LEVEL SO IT WAS NOT BEING SET EVERY TIME!!!
	    ************************************************/
		...
		//PCA0CPL0 = 0xFFFF - PW;
		//PCA0CPH0 = (0xFFFF - PW) >> 8;
		//***********************************************THESE ARE REDUNDANT, DELETED
	}//end if range = far
	if(range<=45){	//if an obstacle is getting near 
		PW=PW_MAX;	//full right swerve to avoid it
		//PCA0CPL0 = 0xFFFF - PW;
		//PCA0CPH0 = (0xFFFF - PW) >> 8;
		//***********************************************THESE ARE REDUNDANT, DELETED
	}//end if range = close
	...
	PCA0CPL0 = 0xFFFF - PW;
	PCA0CPH0 = (0xFFFF - PW) >> 8;	//set servo values
}//end steering servo 
...
void Drive_Motor(void){
	if (range<=10) {
		...
		MOTOR_PW = 2028+read_AD_input(4)*5.8;//adjust speed based on potentiometer
		//***********************************************CONSIDER NOT USING RESULT?!?!?!?!
	}//end else normal ad
	PCA0CPL2 = 0xFFFF - MOTOR_PW;
    PCA0CPH2 = (0xFFFF - MOTOR_PW) >> 8;//set motor values
}//end drive motor
