...
void main(void) {         
	...
	while (1) { 
		...
		while(!SS){	//while the slideswitch is on
			...
			if(hold!=hr_count){
				printf("\n\rR, %u,C, %u,D, %d, s, %d, m, %d,", range, heading/10, desired_heading/10, STEER_PW, MOTOR_PW_AND_STEER_PW);
				hold=hr_count;
			}//end if
...
//-----------------------------------------------------------------------------------
//heading, ranger,and LCD print functions
//-----------------------------------------------------------------------------------
void Heading(void) {
...
//------------------------------------------------------------------------------
//choose gain, heading, and speed functions
//------------------------------------------------------------------------------
void choose_gain(void){ // This function reads desired steering gain from the keypad
	...
	lcd_print("Press 2 for a gain of.5, 3 for 1, 4 for 2");
	keypad=kpd_input(0);
	if(keypad==2)
		steer_gain=1;
	if(keypad==3)
