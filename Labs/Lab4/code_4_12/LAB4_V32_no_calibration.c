...
void main(void) {         
	...
	while (1) { 
		...
		while(!SS)	//while the slideswitch is on
		{
			...
			printf("\n\r Range:%d  Compass:%d  PW:%d, ad %d", range, heading, MOTOR_PW, read_AD_input(4));	//print these on the secure crt for data aquisition
		}
...
void Drive_Motor(void){
	if (range<=10) {
		...
		MOTOR_PW = 2028+(result* 147)/26;
		//MOTOR_PW = 5.8 * result+ 2028;
	}//end else normal ad
