/*	Name: Philip Hoddinott
	Team: Paul McKee Philip Hoddinot, Brian Ortiz-Perez
	Section: 4
	Side: A
	Date: 3/29/2016

	File name: LITEC_HW10_Paul_McKee_Philip_HODDINOTT_BRIAN.c
...
unsigned char ReadCompassChar(void)
{
	...
	return (comp_Data_char[0]);			//simple return statement, just one byte
}//end ReadCompassInt
...
unsigned int ReadCompassInt(void)
{
	...
	return ((unsigned int)(comp_Data_int[0] <<8 | comp_Data_int[1]));		//combine 2 values and return
}//end ReadCompassChar
...
unsigned char ReadRangerChar1(void)
{
	...
	return (range_Data_char1[0]);		//simple return statement, just one byte
}//end ReadRangerChar
